...
def tdoa_to_direction(time_delays):
    ...
    try:
        ...
        return np.array([0, 0, 1])  # Default to pointing up
...
def setup_visualization():
    ...
    # Text display for vector components
    text = ax.text2D(0.05, 0.95, "", transform=ax.transAxes)
    
    return fig, ax, vector, text
...
def update_visualization(frame_num, vector, text):
    ...
    try:
        ...
        try:
            ...
            # Update text display
            text.set_text(f"Direction: [{direction[0]:.2f}, {direction[1]:.2f}, {direction[2]:.2f}]")
            
            # Print to console as well
            print(f"\rDirection: [{direction[0]:.2f}, {direction[1]:.2f}, {direction[2]:.2f}]", end="")
...
def main():
    ...
    try:
        ...
        fig, ax, vector, text = setup_visualization()
