...
def is_sound_active(channels, threshold=30):
    """
...
def setup_visualization():
    ...
    Also adds sliders for microphone gain control.
    """
    ...
    # Add sliders for mic gain adjustment
    mic_gain_sliders = []
    
    # First row of sliders (Mic 0 and 1)
    ax_slider_row1 = fig.add_subplot(gs[1])
    ax_slider_row1.set_title("Microphone Gain Control")
    
    mic0_slider_ax = plt.axes([0.2, 0.65, 0.3, 0.03])
    mic0_slider = plt.Slider(mic0_slider_ax, 'Mic 0', 0.0, 5.0, valinit=mic_gains[0], valstep=0.1)
    
    mic1_slider_ax = plt.axes([0.6, 0.65, 0.3, 0.03])
    mic1_slider = plt.Slider(mic1_slider_ax, 'Mic 1', 0.0, 5.0, valinit=mic_gains[1], valstep=0.1)
    
    # Second row of sliders (Mic 2 and 3)
    ax_slider_row2 = fig.add_subplot(gs[2])
    
    mic2_slider_ax = plt.axes([0.2, 0.55, 0.3, 0.03])
    mic2_slider = plt.Slider(mic2_slider_ax, 'Mic 2', 0.0, 5.0, valinit=mic_gains[2], valstep=0.1)
    
    mic3_slider_ax = plt.axes([0.6, 0.55, 0.3, 0.03])
    mic3_slider = plt.Slider(mic3_slider_ax, 'Mic 3', 0.0, 5.0, valinit=mic_gains[3], valstep=0.1)
    
    # Update function for sliders
    def update_mic_gain(val):
        mic_gains[0] = mic0_slider.val
        mic_gains[1] = mic1_slider.val
        mic_gains[2] = mic2_slider.val
        mic_gains[3] = mic3_slider.val
        print(f"Updated mic gains: {mic_gains}")
    
    # Connect update function to sliders
    mic0_slider.on_changed(update_mic_gain)
    mic1_slider.on_changed(update_mic_gain)
    mic2_slider.on_changed(update_mic_gain)
    mic3_slider.on_changed(update_mic_gain)
    
    mic_gain_sliders.extend([mic0_slider, mic1_slider, mic2_slider, mic3_slider])
    
    plt.tight_layout()
    
    return fig, ax, vector, text, circle, mic_gain_sliders
...
def audio_processor_thread():
    ...
    energy_threshold = 30  # Adjust based on your microphone sensitivity
...
def main():
    ...
    try:
        ...
        fig, ax, vector, text, circle, sliders = setup_visualization()
