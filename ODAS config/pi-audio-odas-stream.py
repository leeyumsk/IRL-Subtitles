        ...
        try:
            while stream_active:
                ...
                try:
                    ...
                    chunk_size = 512 * channels * 2  # 512 samples per frame, with channels * 2 bytes per sample
                    ...
                    try:
                        ...
                        while stream_active and odas_client_connected:
                            ...
                            # ADD ENDIANNESS CONVERSION HERE
                            # Convert the chunk to numpy array, swap bytes, and convert back to bytes
                            # chunk_array = np.frombuffer(chunk, dtype=np.int16)
                            # chunk_array = chunk_array.byteswap()  # Swap bytes to change endianness
                            # chunk = chunk_array.tobytes()
                            
                            # Just before sending to ODAS
                            # print(f"Sending {len(chunk)} bytes, first few bytes: {chunk[:10].hex()}")
                            
                            # Debug info (if you added it)
                            # if frames_sent % 100 == 0:
                            #     print(f"Sending frame {frames_sent}: {len(chunk)} bytes, first 10 bytes: {chunk[:10].hex()}")
