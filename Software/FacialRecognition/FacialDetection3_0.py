                ...
                if frame is not None:
                    ...
                    for i in range(detections.shape[2]):
                        ...
                        if confidence > conf_threshold:
                            ...
                            offset_x = int((x_right_top - x_left_bottom) * 0.15)
                            offset_y = int((y_right_top - y_left_bottom) * 0.1)
                            rect_width = int((x_right_top - x_left_bottom) * 0.8)
                    ...
                    if not face_detected:
                        ...
                        if AudioCapture.AudioProcessor.transcription.strip():
                            text = AudioCapture.AudioProcessor.transcription
                        else:
                            text = "No face no transcribed audio"
...
#faceThread = threading.Thread(target = display_mjpeg_stream(url))
#audioThread = threading.Thread(target = AudioCapture.main())
#audioThread.start()
#faceThread.start()
...
def main():
    ...
    # Start the facial detection thread
    face_thread = threading.Thread(target=display_mjpeg_stream, args=(url,))
    face_thread.start()

    # Wait for threads to finish (you can replace this with a more graceful shutdown if needed)
    audio_thread.join()
    face_thread.join()
